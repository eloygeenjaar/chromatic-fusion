...
class MLP2DDecoder(BaseArchitecture):
    def __init__(self,
        ...
        self._lin1 = nn.Sequential(
            nn.Linear(self._input_size, 128, bias=False),
...
class DCGANDecoder(BaseArchitecture):
    def __init__(self,
        ...
        self._layers = nn.ModuleList([
            nn.ConvTranspose3d(
                self._input_size,
                self._num_features * 16, (3, 4, 3), 1, 0, bias=False),
            ...
            nn.ConvTranspose3d(
                ...
                self._num_features, 4, 2, 1, bias=False),
            # state size. (nc) x 64 x 64
            nn.GroupNorm(8, self._num_features),
            nn.ELU(True),
            nn.ConvTranspose3d(
                self._num_features, 1, 4, 2, 1, bias=False)]
        )
        self.apply(self._init_weights)
...
class DCGANDecoderICA(BaseArchitecture):
    def __init__(self,
        ...
        self._layers = nn.ModuleList([
            nn.ConvTranspose3d(
                self._input_size,
                self._num_features * 16, (3, 3, 3), 1, 0, bias=False),
            nn.GroupNorm(8, self._num_features * 16),
            nn.ELU(True),
            # state size. (ngf*8) x 4 x 4
            nn.ConvTranspose3d(
            ...
            # state size. (ngf) x 32 x 32
            nn.ConvTranspose3d(
                self._num_features * 2, 8, 4, 2, 1, bias=False)]
        )
        self.apply(self._init_weights)
