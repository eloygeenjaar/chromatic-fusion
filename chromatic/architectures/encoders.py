...
class MLP2DEncoder(BaseArchitecture):
    def __init__(self,
        ...
        self._lin1 = nn.Sequential(
            nn.Linear(1378, 512, bias=False),
            nn.GroupNorm(8, 512), nn.ELU(), nn.Dropout(0.05))
        self._lin2 = nn.Sequential(
            nn.Linear(512, 256, bias=False),
            nn.GroupNorm(8, 256), nn.ELU(), nn.Dropout(0.05))
        self._lin3 = nn.Sequential(
            nn.Linear(256, 128, bias=False),
            nn.GroupNorm(8, 128), nn.ELU(), nn.Dropout(0.05))
        self._lin4 = nn.Sequential(
            nn.Linear(128, (self._shared_size + self._priv_size), bias=False),
            nn.Dropout(0.05))
...
class DCGANEncoder(BaseArchitecture):
    def __init__(self,
        ...
        self.layers = nn.Sequential(
            nn.Conv3d(
                1, self._num_features, 4, 2, 1, bias=False),
            nn.ELU(inplace=True),
            # state size. (ndf) x 32 x 32
            nn.Conv3d(
                self._num_features,
                self._num_features * 2, 4, 2, 1, bias=False),
            nn.GroupNorm(8, self._num_features * 2),
            nn.ELU(inplace=True),
            # state size. (ndf*2) x 16 x 16
            nn.Conv3d(
                self._num_features * 2,
                self._num_features * 4, 4, 2, 1, bias=False),
            nn.GroupNorm(8, self._num_features * 4),
            nn.ELU(inplace=True),
            # state size. (ndf*4) x 8 x 8
            nn.Conv3d(
                self._num_features * 4,
                self._num_features * 8, 4, 2, 1, bias=False),
            nn.GroupNorm(8, self._num_features * 8),
            nn.ELU(inplace=True),
            # state size. (ndf*8) x 4 x 4
            nn.Conv3d(
                self._num_features * 8,
                self._num_features * 16, 4, 2, 1, bias=False),
            nn.GroupNorm(8, self._num_features * 16),
            nn.ELU(inplace=True),
            # state size. (ndf*8) x 4 x 4
            nn.Conv3d(
                self._num_features * 16,
                priv_size + shared_size, (3, 4, 3), 1, 0, bias=False)
        )
        self.apply(self._init_weights)
...
class DCGANEncoderICA(BaseArchitecture):
    def __init__(self,
        ...
        self.layers = nn.Sequential(
                ...
                nn.Conv3d(
                    self._num_features * 16,
                    priv_size + shared_size, (3, 3, 3), 1, 0, bias=False)
        )
        self.apply(self._init_weights)
